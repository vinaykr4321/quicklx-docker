...
$ajaxcap_temp = optional_param('ajaxcap_temp', '', PARAM_CLEAN);
$ajaxvalue_temp = optional_param('ajaxvalue_temp', '', PARAM_CLEAN);
...
// check if ajax callback
if ($ajaxcap_temp) {
    error_log('Got it '.$ajaxcap_temp.' '.$ajaxvalue_temp);
    $parts_temp = explode('.', $ajaxcap_temp);
    list($type, $id, $roleid, $capability) = $parts_temp;
    ...
    if ($type == 't') {
    	...
    	$selected_companyid = $DB->get_record_sql("select GROUP_CONCAT(id) as companyid from mdl_company where previousroletemplateid  = $templateid");
    	$myArray = explode(',', $selected_companyid->companyid);
    	foreach ($myArray as $cid) {
        ...
        print_object($restriction);
        

    	 if ($ajaxvalue_temp=='false') {

    	 	echo 'Insert Here';
    	 	if (!$restriction) {
    	 		$restriction = new stdClass();

    	 		echo "No value found --- >  Insert here";
                echo $restriction->companyid = $cid;
                echo $restriction->roleid = $roleid;
                echo $restriction->capability = $capability;
               // print_object($restriction);
...
$(".checkbox").change(function() {
    $.post("<?php echo $linkurl; ?>", {
        ajaxcap_temp:this.value,
        ajaxvalue_temp:this.checked
    });
});
</script>
<?php
