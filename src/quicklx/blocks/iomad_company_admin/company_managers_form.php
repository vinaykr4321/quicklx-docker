...
class company_managers_form extends moodleform {
    ...
    public function definition_after_data() {
        ...
        if (count($this->potentialusers->find_users('')) || count($this->currentusers->find_users(''))) {
            ...
            $mform->addElement('html', '<table summary=""
                                        ...
                                        cellspacing="0">
                <tr>
                  <td id="existingcell" style="width: 44%;">');
            /*
             * Custom code 
            ...
            $mform->addElement('html', '
                  ...
                  <td id="potentialcell" style="width: 44%;">');
    ...
    public function process($departmentid, $roletype) {
        ...
        $companyreportmanagerrole = $DB->get_record('role', array('shortname' => 'companyreportmanager'));
                ...
                foreach ($userstoassign as $adduser) {
                    ...
                    if ($allow) {
                        if ($roletype != 0 && $roletype != 3) {
                            ...
                            if ($userrecord = $DB->get_record('company_users', array('companyid' => $this->selectedcompany,
                                'userid' => $adduser->id,
                                'departmentid' => $departmentid))) {
                                ...
                                } else if ($roletype == 4 && $userrecord->managertype == 0) {
                                    // Give them the department manager role.
                                    role_assign($companyreportmanagerrole->id, $adduser->id, $this->context->id);
                                }
                                if ($roletype != 3) {
                                ...
                                $DB->set_field('company_users', 'managertype', 1, array('userid' => $adduser->id, 'companyid' => $this->selectedcompany, 'departmentid' => $departmentid));
                                ...
                                if ($CFG->iomad_autoenrol_managers) {
                                    ...
                                    if ($companycourses = $DB->get_records('company_course', array('companyid' => $this->selectedcompany))) {
                                        foreach ($companycourses as $companycourse) {
                                            if ($DB->record_exists('course', array('id' => $companycourse->courseid))) {
                                                if ($DB->record_exists('company_created_courses', array('companyid' => $companycourse->companyid,
                                                            'courseid' => $companycourse->courseid))) {
                                                    company_user::enrol($adduser, array($companycourse->courseid), $companycourse->companyid, $companycourseeditorrole->id);
                                                } else {
                                                    company_user::enrol($adduser, array($companycourse->courseid), $companycourse->companyid, $companycoursenoneditorrole->id);
                            ...
                            } else if (($roletype == 1 || $roletype == 2) && !$DB->get_record('company_users', array('companyid' => $this->selectedcompany,
                                        'userid' => $adduser->id,
                                        'departmentid' => $departmentid))) {
                                ...
                                $DB->insert_record('company_users', array('companyid' => $this->selectedcompany,
                                    'userid' => $adduser->id,
                            ...
                            // Assign the user to department as staff.
                            //company::assign_user_to_department($departmentid, $adduser->id);
                            company::assign_user_to_department_custom($departmentid, $adduser->id, $this->selectedcompany);
                        }
                    }
            ...
            if (!empty($userstounassign)) {
                ...
                if ($roletype != 0) {
                    foreach ($userstounassign as $removeuser) {
                        ...
                        $userrecord = $DB->get_record('company_users', array('companyid' => $this->selectedcompany, 'userid' => $removeuser->id, 'departmentid' => $departmentid, 'managertype' => $roletype));
                        ...
                        if ($roletype != 3 &&
                                $DB->get_records_sql("SELECT id FROM {company_users}
                                                  ...
                                                  AND managertype = 1", array('userid' => $removeuser->id,
                                    'companyid' => $this->selectedcompany,
                                    'departmentid' => $departmentid))) {
                            ...
                            if ($roletype != 3) {
                                if ($roletype == 1 || $roletype == 2 || $roletype == 4) {
                                    $checkExistance = $DB->get_record_sql("SELECT * FROM {company_users} WHERE companyid = $this->selectedcompany AND userid = $removeuser->id AND managertype = $roletype ");
                                    if (empty($checkExistance)) {
                                        if ($roletype == 1) {
                                            ...
                                            role_unassign($companyreportmanagerrole->id, $removeuser->id, $this->context->id);
                                        }
                                    }
                                }
                            }
                        ...
                        $eventother = array('companyname' => $company->get_name(),
                            ...
                            'usertype' => $roletype,
                            'usertypename' => $managertypes[$roletype]);
                        $event = \block_iomad_company_admin\event\company_user_unassigned::create(array('context' => context_system::instance(), 'objectid' => $company->id, 'userid' => $removeuser->id, 'other' => $eventother));
                        $event->trigger();
                    }
                } else {
                    foreach ($userstounassign as $removeuser) {
                        ...
                        company::assign_user_to_department_custom($departmentid, $removeuser->id, $this->selectedcompany);
                        ...
                        $event = \block_iomad_company_admin\event\company_user_unassigned::create(array('context' => context_system::instance(),
                                    ...
                                    'other' => $eventother));
                        $event->trigger();
                        if ($DB->count_records('company_users', array('companyid' => $this->selectedcompany, 'userid' => $removeuser->id)) > 1) {
                            $DB->delete_records('company_users', array('companyid' => $this->selectedcompany, 'userid' => $removeuser->id, 'managertype' => $roletype, 'departmentid' => $this->departmentid));
                        }
                    }
                }
 ...
 * (To hide the unwanted dropdown when we select Group manager role)
 */


/*
  $othersselect = new single_select(new moodle_url($linkurl, $urlparams), 'showothermanagers', array(get_string('no'), get_string('yes')), $showothermanagers);
