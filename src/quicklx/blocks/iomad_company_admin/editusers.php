...
if (!$users) {
    ...
    foreach ($users as $user) {
       ...
       /* if ((iomad::has_capability('block/iomad_company_admin:company_course_users', $systemcontext)
        ...
        /* prior link 'blocks/iomad_company_admin/company_users_licenses_form.php' */

        if ((iomad::has_capability('block/iomad_company_admin:company_license_users', $systemcontext)
             or iomad::has_capability('block/iomad_company_admin:editallusers', $systemcontext))
             and ($user->id == $USER->id or $user->id != $mainadmin->id)
             and !is_mnet_remote_user($user)) {
            $url = new moodle_url('/blocks/iomad_company_admin/company_license_users_form.php', array(
                'userid' => $user->id,
            ));
...
function iomad_get_users_listing($sort='lastaccess', $dir='ASC', $page=0, $recordsperpage=0,
    ...
    if ($sort) {
        if ($sort == "department") {
            $sort = "GROUP BY u.id ORDER by d.name $dir";
        } else if ($sort == "company") {
            $sort = "GROUP BY u.id ORDER by c.name $dir";
        } else {
            $sort = "GROUP BY u.id ORDER BY u.$sort $dir";
        }
    }
    ...
    if (!empty($extraparams['showall'])) {
        ...
        if ($parentslist = $company->get_parent_companies_recursive()) {
            $companysql = " AND c.id = cu.companyid AND u.id NOT IN (
                            SELECT userid FROM {company_users}
                            WHERE companyid IN (" . implode(',', array_keys($parentslist)) ."))";
        } else {
            $companysql = " AND c.id = cu.companyid";
        }
           $params['companyid'] = $extraparams['companyid'];
    }
    return $DB->get_records_sql("SELECT concat(c.id, '-', u.id), u.*, GROUP_CONCAT(d.name SEPARATOR ', ') as departmentname, c.name as companyname
