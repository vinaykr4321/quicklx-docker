...
$stdfields = array('id', 'firstname', 'lastname', 'username', 'email',
        ...
        'mnethostid', 'institution', 'department',  'dept1', 'dept2', 'dept3','idnumber', 'skype',
...
if ($proffields = $DB->get_records('user_info_field')) {
    foreach ($proffields as $proffield) {
        $prffields[] = 'profile_field_'.$proffield->name;
    }
    unset($proffields);
}
if (empty($iid)) {
        ...
        while ($line = $cir->next()) {
            ...
            if ($existinguser) {
                ...
                if (!empty($user->department)) {
                 ...
                 // Is the company dept1 valid?

                if (!empty($user->dept1)) {
                    if (!$dept1 = $DB->get_record('department', array('company' => $company->id,
                        'name' => $user->dept1))) {
                        $upt->track('department', get_string('invaliddepartment', 'block_iomad_company_admin'), 'error');
                        $upt->track('status', $strusernotaddederror, 'error');
                        $line[] = get_string('invaliddepartment', 'block_iomad_company_admin');
                    ...
                    // Make sure the user can manage this dept1.
                    if (!company::can_manage_department($dept1->id)) {
                        $upt->track('department', get_string('invaliddepartment', 'block_iomad_company_admin'), 'error');
                        $upt->track('status', $strusernotaddederror, 'error');
                        $line[] = get_string('invaliddepartment', 'block_iomad_company_admin');
                        $errornum++;
                ...
                // Is the company dept2 valid?

                if (!empty($user->dept2)) {
                    if (!$dept2 = $DB->get_record('department', array('company' => $company->id,
                        'name' => $user->dept2))) {
                        $upt->track('department', get_string('invaliddepartment', 'block_iomad_company_admin'), 'error');
                        $upt->track('status', $strusernotaddederror, 'error');
                        $line[] = get_string('invaliddepartment', 'block_iomad_company_admin');
                    ...
                    // Make sure the user can manage this dept2.
                    if (!company::can_manage_department($dept2->id)) {
                        $upt->track('department', get_string('invaliddepartment', 'block_iomad_company_admin'), 'error');
                        $upt->track('status', $strusernotaddederror, 'error');
                        $line[] = get_string('invaliddepartment', 'block_iomad_company_admin');
                        $errornum++;
                        ...
                        continue;
                         ...
                         // Is the company dept3 valid?

                if (!empty($user->dept3)) {
                    if (!$dept3 = $DB->get_record('department', array('company' => $company->id,
                        'name' => $user->dept3))) {
                        $upt->track('department', get_string('invaliddepartment', 'block_iomad_company_admin'), 'error');
                        $upt->track('status', $strusernotaddederror, 'error');
                        $line[] = get_string('invaliddepartment', 'block_iomad_company_admin');
                    ...
                    // Make sure the user can manage this dept3.
                    if (!company::can_manage_department($dept3->id)) {
                        $upt->track('department', get_string('invaliddepartment', 'block_iomad_company_admin'), 'error');
                        $upt->track('status', $strusernotaddederror, 'error');
                        $line[] = get_string('invaliddepartment', 'block_iomad_company_admin');
                        $errornum++;
                ...
                if (!empty($user->department)) {
                    ...
                    //company::assign_user_to_department($department->id, $user->id);
                     company::assign_user_to_department_custom($department->id, $user->id, $company->id);
                } else {
                    company::assign_user_to_department($formdata->userdepartment, $user->id);
                }


                // Do we have a dept1 in the file?
                if (!empty($user->dept1)) {
                    // we have already validated this.  Assign the user.
                 
                    //company::assign_user_to_department($dept1->id, $user->id);
                    company::assign_user_to_department_custom($dept1->id, $user->id, $company->id);
                    
                } else {
                    company::assign_user_to_department($formdata->userdepartment, $user->id);
                }
                
                // Do we have a dept2 in the file?
                if (!empty($user->dept2)) {
                   company::assign_user_to_department_custom($dept2->id, $user->id, $company->id);
                    
                } else {
                    company::assign_user_to_department($formdata->userdepartment, $user->id);
                }
                
                 // Do we have a dept3 in the file?
                if (!empty($user->dept3)) {
                   company::assign_user_to_department_custom($dept3->id, $user->id, $company->id);
            ...
            foreach ($columns as $column) {
                if (preg_match('/^course\d+$/', $column)) {
                    ...
                    $name = $user->{'course'.$i};
                    if (!array_key_exists($name, $ccache)) {
                        if (!$course = $DB->get_record('course', array('name' => $name), 'id, name')) {
                            $upt->track('enrolments', get_string('unknowncourse', 'error', $name), 'error');
                            continue;
                        }
                        $ccache[$name] = $course;
                        $ccache[$name]->groups = null;
                    }
                    company_user::enrol($user, $ccache[$name], $companyid);
                }
...
*/
            ...
            if (!empty($formdata->licenseid)) {
                ...
                foreach ($formdata->licenseid as $lic) {
                 
                   if ($DB->get_record('companylicense', array('id' => $lic, 'program' => 1))) {
                    // This is a program of courses.  Set them!
                    $licensecourses = $DB->get_records_sql_menu("SELECT c.id, clc.courseid FROM {companylicense_courses} clc
                                                                          JOIN {course} c ON (clc.courseid = c.id
                                                                          AND clc.licenseid = :licenseid)",
                                                                          array('licenseid' => $lic));
         $formdata->licensecourses = $licensecourses;

                }

                 $timestamp = time();
                $licenserecord = (array) $DB->get_record('companylicense', array('id' => $lic));
                ...
                foreach ($licensecourses as $licensecourse) {
                   ...
                   // $DB->set_debug(true);
                    ...
                    if ($DB->get_record_sql("SELECT id FROM {companylicense_users}
                                             ...
                                             array('userid' => $user->id, 'licenseid' => $lic,
                    ...
                    if ($allow) {
                       ...
                       // Create an event.
                    $eventother = array('licenseid' => $lic,
                                    'duedate' => $timestamp);

               
                    
                  
                    $event = \block_iomad_company_admin\event\user_license_assigned::create(array('context' => context_course::instance($licensecourse),'objectid' => $lic,
                     'courseid' => $licensecourse,  'userid' => $user->id, 'other' => $eventother));
                  ...
                  updateEnrolmentEndDate($user->id,$licensecourse,$lic);
                        }
                    }
                }
...
while ($fields = $cir->next()) {
    ...
    if ($companyid > 0 && isset($rowcols['profile_field_company']) && !empty($rowcols['profile_field_company'])
        && $rowcols['profile_field_company'] != $companyname ) {
...
Y.on('change', submit_form, '#licenseidselector');
 function submit_form() {
     ...
     var nValue = $('#page-blocks-iomad_company_admin-uploaduser #licenseidselector').val();
     alert(nValue);
    $.ajax({
        type: "GET",
...
class uu_progress_tracker {
    ...
    public function init() {
        ...
        echo '<th class="header c' . $ci++ . '" scope="col">Dept1</th>';
        echo '<th class="header c' . $ci++ . '" scope="col">Dept2</th>';
        echo '<th class="header c' . $ci++ . '" scope="col">Dept3</th>';
        echo '</tr>';
        $this->_row = null;
    }
...
/**
 * Update user_enrolment table with the earliest date for course expiry.
 */
...
function updateEnrolmentEndDate($userid,$courseid,$licenseId)
{
    ...
    $timeendcheck = course_end_date_final($courseid, $userid);
...
function uu_allowed_roles_cache() {
    ...
    foreach ($allowedroles as $rid => $rname) {
        ...
        if (!is_numeric($rname)) { // Only non-numeric names are supported!!!
