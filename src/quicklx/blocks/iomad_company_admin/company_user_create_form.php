...
class user_edit_form extends company_moodleform {
    ...
    */
    ...
    $mform->addElement('hidden', 'use_email_as_username', 1);
        ...
        $managerarray['4'] = get_string('companyrolemanager', 'block_iomad_company_admin');
        }
        ...
        if (iomad::has_capability('block/iomad_company_admin:allocate_licenses', $systemcontext)) {
            ...
            if (count($foundlicenses) == 0) {
                ...
                //$licenseselector = $mform->addElement('select', 'licenseid', get_string('select_license', 'block_iomad_company_admin'), $licenses, array('id' => 'licenseidselector'));

                /* 
                 * Custom code
                 * Author: Syllametrics | support@syllametrics.com
                 * Update: 31st Oct, 2018
                 * Convert "select a license" dropdown to multiselect
                */
                natcasesort($foundlicenses);
                //ksort($licenses);
                $alllicenses = array('0' => get_string('nolicense', 'block_iomad_company_admin')) + $foundlicenses;
                //sort($licenses, SORT_NATURAL | SORT_FLAG_CASE);
                $licenseselector = $mform->addElement('select', 'licenseid', get_string('select_license', 'block_iomad_company_admin'), $alllicenses, array('id' => 'licenseidselector','size' => '15'));
                $licenseselector->setMultiple(true);
                ...
                if (!$licensecourses = $DB->get_records_sql_menu("SELECT c.id, c.fullname FROM {companylicense_courses} clc JOIN {course} c ON (clc.courseid = c.id AND clc.licenseid = :licenseid) ORDER BY c.fullname", array('licenseid' => $mylicenseid))) {
    ...
    public function get_data() {
        ...
        if ($data) {
            ...
            /*if (!empty($data->licenseid) && empty($data->selectedcourses)) 
            {
                $licenseids = implode(',', $data->licenseid);
                $courses = '';
                $courses = $DB->get_records_sql('SELECT courseid FROM {companylicense_courses} WHERE licenseid IN (?)',array($licenseids));
                $data->selectedcourses = array_keys((array) $courses);
            }*/
        }
        return $data;
    }
...
if ($mform->is_cancelled()) {
    ...
    * Enroll user into licese courses
   */

       
    enrolUserToLicenseCourses($data->id,$data->licenseid,$data->companyid);// Userid, licenseid , companyid
...
//    print_object($data);exit;  
    ...
    if (!empty($data->managertype)) {
        ...
        }else if ($data->managertype == 4){
            // Assign the user as a company manager.
            $companymanagerrole = $DB->get_record('role', array('shortname' => 'companyreportmanager'));
            // Give them the manager role.
            role_assign($companymanagerrole->id, $userid, $systemcontext->id);
        }
    }
...
/* gnuwings commented -  Since multiple license selection implimented the below code have error.
The above function call will completed the license allocations 
    // Assign and licenses.
    if (!empty($licenseid)) {
...
Y.on('click', submit_form, '#licenseidselector');
 function submit_form() {
     //var nValue = Y.one('#licenseidselector').get('value');
     var nValue = $('#page-blocks-iomad_company_admin-company_user_create_form #licenseidselector').val();
    $.ajax({
        type: "GET",
...
function enrolUserToLicenseCourses($userid,$licenseids,$companyid)
{
    global $DB;

    foreach ($licenseids as $licenseId)
    {
        $allCourses = $DB->get_records_sql("SELECT courseid from {companylicense_courses} where licenseid = $licenseId");
        if(!empty($allCourses))
        {
            foreach ($allCourses as $course) 
            {
                $check = checkCompanyCourseAccess($course->courseid, $companyid,$licenseId);
                if($check)
                {
                    updateCompanyLicenseUsed($licenseId); // increasing 1 in used
                    if(enrolUser($userid,$course->courseid))
                    {
                        insertCompanyUser($userid,$course->courseid,$licenseId);
...
function checkCompanyCourseAccess($courseid, $companyid,$licenseids) {
    global $DB;
    $access = $DB->get_record_sql("SELECT c.*,cc.courseid FROM {companylicense} AS c JOIN {companylicense_courses} AS cc ON c.id=cc.licenseid WHERE cc.courseid=$courseid AND c.companyid=$companyid AND c.id = $licenseids");
    if ($access) {
...
function insertCompanyUser($userid,$courseid,$licenseid)
{
    ...
    //Gnuwings added to license assigned event trigger
    // Create an event.
        $eventother = array('licenseid' => $licenseid,
                                    'duedate' =>0);
