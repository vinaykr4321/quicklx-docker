...
class company_edit_form extends company_moodleform {
    ...
    public function __construct($actionurl, $isadding, $companyid, $companyrecord, $firstcompany = false, $parentcompanyid = 0, $child = false) {
        ...
        if (empty($this->companyrecord->theme)) {
            $this->companyrecord->theme = 'remui_child';
        }
    ...
    public function definition() {
        ...
        /**
          ...
          * (This code will comment city and country from group management -> create group form)
        ...
        */
        ...
        /**
          * Custom Code
          * Syllametrics | support@syllametrics.com  
          * (This code will comment email secction from group management -> create group form)
          * Updated On: 25th Oct, 2018   
        */
        ...
        if(empty($templates[$this->previousroletemplateid])){
            $templates[$this->previousroletemplateid] = 'None';
        }
        ...
        // This code will add Unassigned/Unlicensed options in ccompany edit form for catalog page
        // auther : 2018 Syllametrics | support@syllametrics.com

        $mform->addElement('selectyesno', 'unassigned', 'Show unassigned courses in catalog');
        $mform->setDefault('unassigned', 1);

        $mform->addElement('selectyesno', 'unlicensed', 'Show unlicensed courses in catalog');
        $mform->setDefault('unlicensed', 1);
        
        $mform->disabledIf('unlicensed', 'unassigned', 0);
        ...
        //Checkbox to check expiry courses
        $expiredcourse_getinfo = isset($this->companyrecord->expiredcourse) ? 1 : 0;
        $mform->addElement('checkbox', 'expiredcourse', get_string('expiredcourse', 'block_iomad_company_admin'),'Enable');

        //$company_expiredcourse = optional_param('createnew', 1, PARAM_INT);
        if(optional_param('createnew', 0, PARAM_INT)){
            $mform->setDefault('expiredcourse', 1);
        }else{
            $mform->setDefault('expiredcourse', $expiredcourse_getinfo);
        }
        //Custom code end
        ...
        //Checkbox to check enable registration key
        $enableregkey_getinfo = isset($this->companyrecord->enableregkey) ? 1 : 0;

     
        $mform->addElement('checkbox', 'enableregkey',get_string('enableregkey', 'block_iomad_company_admin'),'Enable');
       
     

        if(optional_param('createnew', 0, PARAM_INT)){
            $mform->setDefault('enableregkey', 1);
        }else{
            $mform->setDefault('enableregkey', $enableregkey_getinfo);
        }
        //Custom code end
        ...
        /**
          * Custom Code
          * Syllametrics | support@syllametrics.com  
          * (This code will comment "user default" secction from group management -> create group form)
          * Updated On: 25th Oct, 2018   
        */
        ...
        try {
            ...
            foreach ($theme->parents as $parentstheme) {
                if($parentstheme == 'iomad' || $parentstheme == 'bootstrap' || $parentstheme == 'remui'){
                    $ischild = true;
                    break;
                }
            }
        ...
        if (iomad::has_capability('block/iomad_company_admin:company_edit_appearance', $context) ||
            ...
            if ( preg_match('/iomad/', $this->companyrecord->theme) || $ischild) {
                 ...
                 * Last Update : 12th Nov, 2018
                 * Option to insert splash image (image at login page)
                */

                $mform->addElement('filemanager', 'splashimage',
                                    get_string('splash', 'block_iomad_company_admin'), null,
                                    array('subdirs' => 0,
                                         'maxbytes' => 150 * 1024,
                                         'maxfiles' => 1,
                                         'accepted_types' => array('*.jpg', '*.gif', '*.png')));
            ...
            /*    $mform->addElement('iomad_colourpicker', 'maincolor', get_string('maincolor', 'block_iomad_company_admin'), 'size="20"');
                 ...
                 * This code will add colour picker for side nav
                */
                $mform->addElement('iomad_colourpicker', 'leftcolumncolor', 'Left column and course tile color', 'size="20"');
                $mform->setType('leftcolumncolor', PARAM_CLEAN);

                /*
                 * Custom Code
                 * 2018 Syllametrics | support@syllametrics.com
                 * Last Update : 11 Dec, 2018
                 * This code will set login page background colour and button colour
                */
                $mform->addElement('iomad_colourpicker', 'loginbackground', 'Login page background color', 'size="20"');
                $mform->setType('loginbackground', PARAM_CLEAN);

                $mform->addElement('iomad_colourpicker', 'loginbutton', 'Login button color', 'size="20"');
                $mform->setType('loginbutton', PARAM_CLEAN);
    ...
    public function validation($data, $files) {
        ...
        // Custom Code - Syllametrics - Jan 29 2019 - Check for domain existence
        if (!empty($data['hostname'])){
             if (!empty($this->companyid)) {
                            $company = $DB->get_record('company', array('id' => $this->companyid));
                            $companieshostname = $company->hostname;
                            if(strcmp($data['hostname'],$companieshostname) == 0)
                                    return $errors;

                    }
                  
                     $check = check_hostname($data['hostname']) ;
                  if($check == 0){
                         $errors['hostname'] = get_string('companydomainexist',
                                                     'block_iomad_company_admin');
                    }
...
if ($mform->is_cancelled()) {
    ...
    $data->userid = $USER->id;
    //$data->leftcolumncolor = $data->leftcolumncolor;
//print_object($data);exit;
    if ($isadding) {
        ...
        /**
          * Custom Code
          * Syllametrics | support@syllametrics.com  
          * (This code related to the above commented code of city and country)
          * Updated On: 25th Oct, 2018   
        */
        ...
        $data->id = $companyid;
        // Custom code - Syllametrics - Jan 29 2019 - Populate the custom table for hostname tracking
    	if(isset($data->hostname) && $data->hostname !=""){
    		$host_data = new stdClass();
    		$host_data->companyid = $data->id;
    		$host_data->hostname = $data->hostname;
    		$host_data->status="added";
    		$host_data->htaccess=0;
    		$host_data->timecreated=time();
    		$host_data->timemodified=time();
    		$hostnameid = $DB->insert_record('iomad_company_hostname_trac', $host_data);
    	}
	    // End custom code
        ...
        $DB->update_record('company', $data);
        // Custom Code - Syllametrics - Jan 29 2019 - update the status as updated iomad_company_hostname_trac
        $trac = $DB->get_record('iomad_company_hostname_trac', array('companyid' => $companyid));
        $istractable = $DB->get_records('iomad_company_hostname_trac');
        if($trac){
            $host_data = new stdClass();
            $host_data->id = $trac->id;
            $host_data->hostname = $data->hostname;
            if($data->hostname != $trac->hostname){
                $host_data->status="updated";
                $host_data->htaccess=0;
            }   
            $host_data->timemodified=time();
            $DB->update_record('iomad_company_hostname_trac', $host_data);
        }
        else if($istractable && isset($data->hostname) && $data->hostname !=""){
            $host_data = new stdClass();
            $host_data->companyid = $data->id;
            $host_data->hostname = $data->hostname;
            $host_data->status="updated";
    ...
    if (!empty($data->splashimage)) {
        file_save_draft_area_files($data->splashimage,
                                   $context->id,
                                   'theme_iomad',
                                   'splashimage',
                                   $data->splashimage,
    ...
    // Syllametrics (support@syllametrics.com)
    // Last update: May 2019
    // Custom check to update expirycourses, unlicensed and unassigned colum of company table
    $expiredcourse_info = isset($data->expiredcourse) ? 1 : 0;
    $enableregkey_info = isset($data->enableregkey) ? 1 : 0;
  
    $DB->execute("UPDATE {company} SET `unlicensed` = $data->unlicensed, `unassigned` = $data->unassigned, `expiredcourse` = $expiredcourse_info,`enableregkey` = $enableregkey_info WHERE id = $companyid");
