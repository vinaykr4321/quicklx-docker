...
class admin_uploaduser_form1 extends company_moodleform {
    public function definition () {
        ...
        $linkcontent = '<a href="csv/Users.csv" style="float:right; margin-top:-50px; padding-right:20px;">Download CSV template for user upload</a>';
        $mform->addElement('static', 'samplecsv', '', $linkcontent);
    ...
    public function definition_after_data() {
        ...
        */
        ...
        if (iomad::has_capability('block/iomad_company_admin:allocate_licenses', $systemcontext)) {
             ...
             * Update: 25th Feb, 2019
             * Convert dropdown to multiselect
            */
           /* $arrVal = $licenses[0];
            natcasesort($licenses);
            unset($licenses[0]);
            array_unshift($licenses, $arrVal);*/
 	    $arrVal = $licenses[0];
            asort($licenses);
            $orderid= array();
            $orderid[] = $arrVal;
            foreach ($licenses as $key => $orderby) {
                if($key != 0)
                $orderid[$key] =$orderby;
            }
            unset($licenses[0]);
            array_unshift($licenses, $arrVal);
            $multiple = $mform->addElement('select', 'licenseid', get_string('select_license', 'block_iomad_company_admin'), $orderid, array('id' => 'licenseidselector','size'=>'10','style'=>'margin-top:1%;'));
