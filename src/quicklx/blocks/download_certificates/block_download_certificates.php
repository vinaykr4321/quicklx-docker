...
class block_download_certificates extends block_base {
    ...
    public function get_content() {
        ...
        $sql = "SELECT f.id AS fid, f.userid AS fuserid,f.component AS component, u.firstname, u.lastname, f.contextid AS fcontextid, f.filename AS ffilename,
        ...
        if (empty($certificates)) {
            ...
            $reportpage = $CFG->wwwroot . "/blocks/download_certificates/report.php";
            $this->content->text = get_string('download_certificates_nocertsissued', 'block_download_certificates');
            $this->content->text .= "<br><a href='" . $reportpage . "'><button style='margin-left:1%;'>Check All Certificate</button></a>";
            return $this->content;
        } else {
            foreach ($certificates as $certdata) {
                ...
                $filelink = file_encode_url($CFG->wwwroot . '/pluginfile.php', '/'
                        . $contextid . '/'.$component.'/issue/' . $certificateissueid . '/' . $filename);
            ...
            $this->content->footer = html_writer::link(new moodle_url('/blocks/download_certificates/report.php', array('userid' => $USER->id)), get_string('download_certificates_footermessage', 'block_download_certificates'));
        }
