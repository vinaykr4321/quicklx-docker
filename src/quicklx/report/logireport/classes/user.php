...
/*
	...
	function totalUser($firstname,$lastname,$email,$orderbyName,$sortingOrder,$companyid,$selectPerPage,$page,$perpage,$firstaccessfrom,$firstaccessto,$lastaccessfrom,$lastaccessto)
	{
		...
		global $DB;
		$data = $DB->get_records_sql("SELECT u.id,u.username,u.firstname,u.lastname,c.name,u.timecreated,u.firstaccess,u.lastaccess,u.email FROM {user} AS u LEFT JOIN {company_users} AS cu ON cu.userid=u.id LEFT JOIN {company} AS c ON c.id=cu.companyid $param $companyid ORDER BY $orderbyParam");
	}
