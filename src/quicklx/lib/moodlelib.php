...
function get_login_url() {
    ...
    return "$CFG->wwwroot/login/index.php$param";
}
...
function require_login($courseorid = null, $autologinguest = true, $cm = null, $setwantsurltome = true, $preventredirect = false) {
    ...
    if ($course->id == SITEID) {
        ...
        if (!$access) {
			...
			// Support customized display of courses in catalog and dashboard
            //redirect($CFG->wwwroot .'/enrol/index.php?id='. $course->id);
			if(!$DB->get_record_sql("SELECT * FROM {user_lastaccess} where userid = $USER->id AND courseid = $course->id")) {
            	redirect($CFG->wwwroot .'/enrol/index.php?id='. $course->id);
        	}
        }
    }
...
function delete_user(stdClass $user) {
	...
	/*
         ...
         * Date : March 19, 2019
         * Remove companylicense_users entries from IOMAD licenses when user is deleted
        */

   // Remove users from companylicense_users.
    $DB->delete_records('companylicense_users', array('userid' => $user->id));
...
function delete_course($courseorid, $showfeedback = true) {
     ...
     * Convert Recalculate license allocation numbers when licensed courses are deleted
    */
    $DB->delete_records("company_course", array("courseid" => $courseid)); 

	$licensecoursesall = $DB->get_records('companylicense_courses', array('courseid' => $courseid));


    	foreach ($licensecoursesall as $lcourse) {
    		$lcourse = $lcourse->licenseid;

    		$licensecoursesall_count = $DB->get_records('companylicense_courses', array('licenseid' => $lcourse));

    		$count = count($licensecoursesall_count);
    		$update_allocation = ($count -1);
    		$getalllicense = $DB->get_records('companylicense', array('id' => $lcourse));

    	 	 if($getalllicense[$lcourse]->licenseformat == 0 ){


    	 	$allocation_value = $getalllicense[$lcourse]->allocation / $count;

    		$allocation_value = $allocation_value * $update_allocation;
	
    		$DB->execute("Update {companylicense} set allocation = $allocation_value WHERE id = $lcourse");
    		}
   	 }

    $DB->delete_records("companylicense_courses", array("courseid" => $courseid)); 
...
function send_confirmation_email($user, $confirmationurl = null) {
    ...
    // Directly email rather than using the messaging system to ensure its not routed to a popup or jabber.
    echo "email2";var_dump($message);exit;
