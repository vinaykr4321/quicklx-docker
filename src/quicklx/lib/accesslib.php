...
function reset_role_capabilities($roleid) {
    ...
    $DB->delete_records('role_capabilities', array('roleid' => $roleid, 'contextid' => $systemcontext->id));
...
function get_role_contextlevels($roleid) {
    global $DB;
    return $DB->get_records_menu('role_context_levels', array('roleid' => $roleid), 'contextlevel', 'id,contextlevel');
}
    ...
    if ((empty($roleid) || is_array($roleid)) && strpos($fields, 'ra.id') !== 0) {
        ...
        if (!empty($roleid)) {
            ...
            foreach ($roleid as $id) {
                ...
                $users = $users + get_role_users($id, $context, $parent, $fields, $sort, $all, $group, $limitfrom, $limitnum, $extrawheretest, $whereorsortparams);
            }
            return $users;
        }
    }
...
function get_roles_on_exact_context(context $context) {
    ...
    return $DB->get_records_sql("SELECT r.*
                                   FROM {role_assignments} ra, {role} r
                                  WHERE ra.roleid = r.id AND ra.contextid = ?", array($context->id));
}
...
function get_roles_with_override_on_context(context $context) {
    ...
    return $DB->get_records_sql("SELECT r.*
                                  ...
                                  WHERE rc.roleid = r.id AND rc.contextid = ?", array($context->id));
}
...
function get_capabilities_from_role_on_context($role, context $context) {
    ...
    return $DB->get_records_sql("SELECT *
                                  ...
                                  WHERE contextid = ? AND roleid = ?", array($context->id, $role->id));
}
...
function role_get_name(stdClass $role, $context = null, $rolenamedisplay = ROLENAME_ALIAS) {
    ...
    if (trim($role->name) !== '') {
        ...
        switch ($role->shortname) {
            case 'manager': $original = get_string('manager', 'role');
                break;
            case 'coursecreator': $original = get_string('coursecreators');
                break;
            case 'editingteacher': $original = get_string('defaultcourseteacher');
                break;
            case 'teacher': $original = get_string('noneditingteacher');
                break;
            case 'student': $original = get_string('defaultcoursestudent');
                break;
            case 'guest': $original = get_string('guest');
                break;
            case 'user': $original = get_string('authenticateduser');
                break;
            case 'frontpage': $original = get_string('frontpageuser', 'role');
                break;
            // We should not get here, the role UI should require the name for custom roles!
            default: $original = $role->shortname;
                break;
        }
    }
...
function role_cap_duplicate($sourcerole, $targetrole) {
    ...
    $caps = $DB->get_records_sql("SELECT *
                                    FROM {role_capabilities}
                                   WHERE roleid = ? AND contextid = ?", array($sourcerole->id, $systemcontext->id));
    // adding capabilities
    foreach ($caps as $cap) {
        unset($cap->id);
