...
class global_navigation extends navigation_node {
    ...
    public function initialise() {
        ...
        // Syllametrics - Sept 2018 - For CareerAcademy - Always show link to course catalog
	...
	$this->rootnodes['currentcourse']->mainnavonly = true;
	    ...
	    // JD - Syllametrics: Temporary - don't remove mycourses
            /** @var navigation_node $node */
            
            if ($node->key !== 'home' && $node->key !== 'mycourses' && !$node->has_children() && !$node->isactive) {
                $node->remove();
            }
        }
...
class flat_navigation extends navigation_node_collection {
    ...
    public function initialise() {
        ...
        if ($course->id > 1) {
            ...
            // Support to track/display expired courses correctly
            $companyid = iomad::get_my_companyid(context_system::instance());

            $expicourseId = $DB->get_record_sql("SELECT e.courseid,ue.* FROM {user_enrolments} ue join {enrol} e ON ue.enrolid = e.id where ue.userid = $USER->id AND e.courseid = $course->id AND  ue.timeend != 0");

            if($expicourseId->timeend > time()){
