...
class company {
    ...
    public function get_managertypes() {
        ...
        if (iomad::has_capability('block/iomad_company_admin:assign_department_manager', $systemcontext)) {
            ...
            $returnarray['4'] = get_string('companyrolemanager', 'block_iomad_company_admin');
        }
...
return true;
     ...
     * Handles situation where logged in user who is uploading has >1 manager role
     **/
   public function get_userlevel2($user) {
        ...
        elseif ($userdepartment = $DB->get_records('company_users', array('userid' => $user->id, 'companyid' => $this->id, 'managertype' => 2))) {

            $test = array();
            foreach ($userdepartment as $value) {
                $userlevel = $DB->get_record('department', array('id' => $value->departmentid));
                $test[] = $userlevel;
            }
                     
            
            return $test;
        }elseif ($userdepartment = $DB->get_record('company_users', array('userid' => $user->id, 'companyid' => $this->id))) {
            $userlevel = $DB->get_record('department', array('id' => $userdepartment->departmentid));
            return $userlevel;
        } else {
            return false;
    ...
    public function get_userlevel($user) {
         ...
         * Added if condition for group manager (manager type = 1) to display all users of a company
        */

        if ($userdepartment = $DB->get_record('company_users', array('userid' => $user->id, 'companyid' => $this->id, 'managertype' => 1))) {
            $userlevel = $DB->get_record('department', array('id' => $userdepartment->departmentid));
            return $userlevel;
        }elseif ($userdepartment = $DB->get_record('company_users', array('userid' => $user->id, 'companyid' => $this->id, 'managertype' => 2))) {
            $userlevel = $DB->get_record('department', array('id' => $userdepartment->departmentid));
            return $userlevel;
        }elseif ($userdepartment = $DB->get_record('company_users', array('userid' => $user->id, 'companyid' => $this->id))) {
            $userlevel = $DB->get_record('department', array('id' => $userdepartment->departmentid));
...
*/
    ...
    /**
     * Custom code - Syllametrics - support@syllametrics.com - March 29 2019
     *
     * Gets all of the users that manager is responsible for, handles managers
     * with multiple manager roles
     *
     * Parameters -
     *             $companyid = int;
     *             $departmentid = int;
    ...
    public static function get_my_users($companyid=0, $departmentid=0) {
        ...
        if (empty($departmentid)) {
            ...
            if (is_siteadmin($USER->id)) {
                ...
                $department = $company->get_userlevel2($USER);
                $count = count($department);
                if($count > 1){
                    $deptid = array();
                    foreach ($department as $value2) {
                        $departmentid = $value2->id;
                        $deptid[] = $departmentid;
                    }
                } else {
                    $departmentid = $department->id;
        ...
        if($count > 1){
        $dept_user = array();
            foreach ($deptid as $key => $value3) {
                $a = self::get_recursive_department_users($value3);
                $dept_user[$key] = $a;
            }
            $dept_user_list = self::array_flatten($dept_user);
        } else {
           $dept_user_list = self::get_recursive_department_users($departmentid);
        }

        return $dept_user_list;
    }
 ...
 * Code created to handle user role
*/


    public static function assign_user_to_department_custom($departmentid, $userid, $companyid, $managertype = 0, $ws = false) {
        global $DB;
        $userrecord = array();
        $userrecord['departmentid'] = $departmentid;
        $userrecord['userid'] = $userid;
        // Moving a user.
        if ($currentuser = $DB->get_record('company_users', array('userid' => $userid,'companyid'=>$companyid,'departmentid'=>$departmentid))) 
        {

            $currentuser->departmentid = $departmentid;
            $count = $DB->count_records('company_users', array('userid'=>$userid));
            if ($count >1 && $currentuser->managertype == 0 ) 
            {
                if ($currentuser->status == 1) 
                {
                    $DB->execute("DELETE FROM {company_users} WHERE id=$currentuser->id");
                    return true;
                }
                else
                {
                    $currentuser->status = 1;
                    $DB->update_record('company_users', $currentuser);
                }
                //echo 'if';exit;
            }
            else
            {
                $currentuser->managertype = 0;
                $currentuser->educator = 0;
                $DB->update_record('company_users', $currentuser);
            }
        ...
        {
            $DB->insert_record('company_users', array('companyid' => $companyid,
                                                     'userid' => $userid,
...
return false;
    ...
    public static function check_canedit_user($companyid, $userid) {
        ...
        /**
         * Custom code - Syllametrics - support@syllametrics.com - April 4 2019
         *
         * Updated to correctly handle managers with multiple manager roles
         *
         **/ 

        if ($userrec = $DB->get_record('company_users', array('companyid' => $companyid, 'userid' => $userid))) {

            // Check the current user is a manager or not and what levels they can edit.
            if ($manrec = $DB->get_records('company_users', array('companyid' => $companyid, 'userid' => $USER->id))) {
                 foreach ($manrec as $value) {
                    //print_r($value->managertype);
                    if($value->managertype == 1){
                        $checkauth = 1;
                    }elseif($value->managertype == 2){
                        $checkauth = 1;
                    }
                }


                if (empty($manrec->managertype) && $checkauth != 1) {
                    return false;
                ...
                if (empty($currentcourses[$oldcourseid])) {
                  ...
                  /*  // Deal with enrolments.
                    if ($enrolments = $DB->get_records_sql("SELECT e.id FROM {enrol} e
                        ...
                        /* Custom changes - Delete users from enrolements where license will delete the courses*/
  

                    $deletelicusers = $DB->get_records_sql("SELECT DISTINCT userid
                        FROM {companylicense_users}
                        WHERE licenseid = :licenseid
                        AND licensecourseid = :licensecourseid",
                        array('licensecourseid' => $oldcourseid, 'licenseid' => $licenseid));   
                    ...
                    if($deletelicusers){
                        foreach ($deletelicusers as $deletelicuser) {


                      // Create an event.
                            $eventother = array('licenseid' => $licenseid,
                                'duedate' => 0);
                            $eventunassign = \block_iomad_company_admin\event\user_license_unassigned::create(array('context' => context_course::instance($oldcourseid),
                                'objectid' => $licenseid,
                                'courseid' => $oldcourseid,
                                'userid' => $deletelicuser->userid,
                                'other' => $eventother));
                            $eventunassign->trigger();
                      ...
                      array('licenseid' => $licenseid))) {
                        ...
                        foreach ($licusers as $licuser) {
                            $userlic = array('licenseid' => $licenseid,
                               ...
                               'issuedate' => time());
                            $userlic['id'] =  $DB->insert_record('companylicense_users', $userlic);

                            /*--------- Custom Changes---- Event functionality added when parent add more courses */

                            if($licenserecord->instant == 1){
                            ...
                            $eventother = array('licenseid' => $licenseid,
                                'duedate' => $duedate);
                            $eventupdate = \block_iomad_company_admin\event\user_license_assigned::create(array('context' => context_course::instance($currentcourse->courseid),
                              'objectid' => $userlic['id'],
...
return true;
	...
	// Custom code - Syllametrics - June 2019 - Prevent removal of grade data from active grade table because
	// it is displayed to expired users in the system
    ...
    /**
     * Triggered via user_license_shortened event.
     *
     * @param \block_iomad_company_user\event\user_license_shortened $event
     * @return bool true on success.
     */
    public static function user_license_shortened(\block_iomad_company_admin\event\user_license_shortened $event) {
        global $DB, $CFG;

        $userid = $event->userid;
        $licenseid = $event->objectid;

        $coursealllist = array_keys($DB->get_records_sql("SELECT courseid FROM {companylicense_courses} where licenseid =  $licenseid"));

        foreach ($coursealllist as  $courseid) {
            $ue = $DB->get_records_sql("SELECT ue.timeend FROM {user_enrolments} ue join {enrol} en ON ue.enrolid = en.id where en.courseid = $courseid and ue.userid = $userid");
            if($ue){ 
                    $getexpirydate = self::course_end_date($courseid,$userid);
                    $timeend = strtotime(date("Y-m-d 23:59:59", $getexpirydate));
                    self::updateuserenrolment($courseid,$userid,$timeend);
            }
        }
    ...
    /**
     * Triggered via user_license_extended event.
     *
     * @param \block_iomad_company_user\event\user_license_extended $event
     * @return bool true on success.
     */
    public static function user_license_extended(\block_iomad_company_admin\event\user_license_extended $event) {
        global $DB, $CFG;
      ...
      /**
     * Triggered via user_license_renewed event.
     *
     * @param \block_iomad_company_user\event\user_license_renewed $event
     * @return bool true on success.
     */
    public static function user_license_renewed(\block_iomad_company_admin\event\user_license_renewed $event) {
        global $DB, $CFG;
     ...
     *
     **/
    public static function course_end_date($courseid,$userid) {
         global $DB;
 ...
 /**
     * Update user's course end date
     *
     * Parameters -
     *              $courseid = int;
     *              $userid = int;
     *
     * Returns true if successful to enrol else returns false.
     *
     **/
    public static function updateuserenrolment($courseid,$userid,$timeend){
        global $DB;
      
    $enrolid = $DB->get_record_sql("SELECT id FROM {enrol} WHERE courseid = $courseid AND enrol ='license'");
    $DB->execute("UPDATE {user_enrolments} SET timeend = $timeend WHERE enrolid=$enrolid->id AND userid = $userid");
    }
