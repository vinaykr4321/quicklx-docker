...
// Url stuff.
$url = new moodle_url('/local/report_reportcard/index.php');
...
$url = new moodle_url('/local/report_reportcard/index.php', $params);
...
if($params['daterange'] !='no'){
	...
	echo '<h5>'.$reportdate.'</h5><br>
	<hr style="background-color: black;margin-left: -35%;">
	<br>';
	//print_r($params);
	// Set up the filter form.
	$mform = new iomad_reportcard_filter_form($params);
...
$sql = "select distinct(c.id),name from {company} c 
		...
		$userurl = new moodle_url('/local/report_reportcard/learner_card.php', $params);

		$useraction = html_writer::link($userurl, $user->username,array('title'=>'Learner Report Card'));  
 	...
 	echo '<div id="exportbuttons" style="text-align: center;">	';
	  ...
	  $PAGE->set_url('/local/base/download_excel.php',array('name'=>'reportcard'));
    echo "<br>".$OUTPUT->single_button(new moodle_url($PAGE->url, array('exceldata' => json_encode($array_excel))),  get_string("downloadexcel", 'local_base'));
     ...
     $PAGE->set_url('/local/base/download_csv.php',array('name'=>'reportcard'));
    ...
    $PAGE->set_url('/local/base/download_pdf.php',array('name'=>'reportcard'));
	...
	echo "</div><br />"; 
		...
		echo $output->paging_bar($totalcount, $page, $perpage, new moodle_url('/local/report_reportcard/index.php', $params));
		echo "<br />"; 
