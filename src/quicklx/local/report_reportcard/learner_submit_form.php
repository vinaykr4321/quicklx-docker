...
if(isset($_POST['contactFrmSubmit']) 
	...
	if (isset($_POST['completionstatus']) && $_POST['completionstatus']) {
		$params['completionstatus'] =$_POST['completionstatus'];
	}
		...
		if($format=='pdf'  || $format=='html'){
			...
			$html = $reportdaterange.'<br>'.$reportdate.'<br> User: '.$userdetails;
                ...
                if($format=='pdf' ){
                        ...
                        while (true) {
                                $filename = uniqid('reportcardresults', true) . '.pdf';
			...
			base::create_tempdf($reporthead,$html,$tablepdf,$filename);
                ...
                if($format =='csv' || $format =='xlsx'){
                        $fileLocation = $CFG->tempdir;
                        while (true) {
                                $filename = uniqid('reportcardresults', true) . '.csv';
                                if (!file_exists($fileLocation .'/'. $filename)) break;
                        }
                ...
                if($format =='xlsx')
                {
                    ...
                    $filename = "reportcardresults_". date('Y-m-d-H-i-s') . '.xlsx';
