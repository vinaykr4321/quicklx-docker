...
class EmailVars {
    ...
    public function __construct($company, $user, $course, $invoice, $classroom, $license, $sender, $approveuser) {
        ...
        if (!empty($user->id)) {
	//Added by Syllametrics - support@syllametrics.com - Set URL based on user company
		if(!empty($this->company->hostname)){
			global $CFG;
        	        $wwwurl = strstr($CFG->wwwroot, '.');
	                $domain = $this->company->hostname.$wwwurl;
                        $protocol = "http://";
                        if(is_https) {
                                $protocol = "https://";
                        }
                        $this->url = new moodle_url($protocol.$domain.'/my/');	
		}//till here
	        else
