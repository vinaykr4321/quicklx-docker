...
iomad::require_capability('local/report_course_completion:view', $context);
...
// Url stuff.
$url = new moodle_url('/local/report_course_completion/index.php');
$dashboardurl = new moodle_url('/local/iomad_dashboard/index.php');

// Page stuff:.
$strcompletion = get_string('pluginname', 'local_report_course_completion');
$PAGE->set_url($url);
...
$url = new moodle_url('/local/report_course_completion/index.php', $params);
...
// Only print the header if we are not downloading.
	...
	if($params['daterange'] !='no'){
		$reportdate = 'Date Range: '. get_string($params['daterange'], 'local_base').'<br>';
	...
	$mform = new iomad_course_completion_filter_form($params);
...
if($exportuserdataobj ){
	...
	foreach($exportuserdataobj as $user){
		...
		$tablepdf  .= '<tr style="'.$style.'">
						<td class="cell c0" style="text-align:left;">'.$user->firstname.' </td>
						<td  class="cell c1" style="text-align:left;">'.$user->lastname.'</td>
						<td class="cell c2" style="text-align:left;">'.$user->username.' </td>
						<td class="cell c2" style="text-align:left;">'.$user->address.' </td>
						<td class="cell c2" style="text-align:left;">'.$user->city.' </td>
						<td class="cell c2" style="text-align:left;">'.$showcountry.' </td>
						<td colspan="2" class="cell c2" style="text-align:left;">'.$user->email.' </td>
						<td class="cell c2" style="text-align:left;">'.$user->phone2.' </td>
						<td class="cell c2" style="text-align:left;">'.$user->fullname.' </td>
						<td class="cell c4" style="text-align:left;">'.date('m/d/Y',$user->timecompleted).'</td>
						<td class="cell c2" style="text-align:left;">'.$user->cidnumber.' </td>
						<td class="cell c2" style="text-align:left;">'.$orgname.' </td>
						<td class="cell c4" style="text-align:left;">'.date('m/d/Y',$lastupdated->timecreated).'</td>
						</tr>';
		$i++;
...
if($userdataobj){
    ...
    $coursecomptable->align = array('left', 'center', 'center', 'center', 'center', 'center', 'center', 'center', 'center', 'center', 'center', 'center', 'center');
	...
	echo '<div id="exportbuttons" style="text-align: center;">	';
	  ...
	  $PAGE->set_url('/local/base/download_excel.php',array('name'=>'coursecompletion'));
    echo "<br>".$OUTPUT->single_button(new moodle_url($PAGE->url, array('exceldata' => json_encode($array_excel))),  get_string("downloadexcel", 'local_base'));
     ...
     $PAGE->set_url('/local/base/download_csv.php',array('name'=>'coursecompletion'));
    ...
    $PAGE->set_url('/local/base/download_pdf.php',array('name'=>'coursecompletion'));
	...
	echo $output->paging_bar($totalcount, $page, $perpage, new moodle_url('/local/report_course_completion/index.php', $params));
	echo "<br />"; 
			 ...
			 <input type="hidden" name="userarrayid" value="'. $userarrayid. '">
                   ...
                   echo    ' </select>
                    ...
                    <div class="form-group">
                        ...
                        placeholder="Enter email subject" value="Course completion report"/>
                    </div>
                    <div class="form-group">
 ...
 <div class="modal fade" id="schedulereportForm" role="dialog">
    <div class="modal-dialog">
        <div class="modal-content">      <!-- Modal Header -->
			...
			<input type="hidden" name="reportname" value="course_completion">
			<input type="hidden" name="screen" value="1">
			  <div class="form-group">
                        <label for="description">Schedule Description:</label>
                        <input type="text" class="form-control" id="description" 
                        placeholder="Enter Description" value=" Course Completion"/>
                    </div>
                      <div class="form-group">
                        <label >Report Format:</label><br>
                   ...
                   echo    ' </select>
                     ...
                     <div class="form-group">
                        ...
                        placeholder="Enter email subject" value="Course Completion"/>
                    </div>
                    <div class="form-group">
