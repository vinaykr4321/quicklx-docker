...
class report_completionstatus {
	
	public static function get_all_users_courses($userid,$params)
	{
		global $DB;
		...
		if(!empty($params['license'])){
			...
			$sql ="select distinct(cc.courseid) from {companylicense} cl 
					join {companylicense_courses} cc on cc.licenseid=cl.id
					where cl.id=".$license;
					$courselicenses = $DB->get_records_sql($sql);	
					foreach($courselicenses as $courselicense){
						$lcourse[]=$courselicense->courseid;
					}	
					$lcourse= implode(',',$lcourse);	
					$licensesearch = " AND c.id in ($lcourse) ";
		}
		if(isset($params['daterange']) && $params['daterange'] =='datecomp' ){
				$beginOfDay = strtotime("midnight", $params['datefrom']);
				$endOfDay   = strtotime("tomorrow", $params['dateto']) - 1;
				
				$sql = "SELECT cc.id as ccid,cc.userid as userid,c.* 
								FROM {course_completions} cc
								JOIN {user} u ON (u.id = cc.userid)
							   JOIN {course} c ON (cc.course = c.id)
							   WHERE    cc.userid in ($userid)
							   $coursesearch $licensesearch
			  ...
			  $sql = "SELECT ue.id as ueid,ue.timecreated as registered,ue.userid as userid,
								   ...
								   WHERE ue.userid in ($userid) ".
								   $courseregisterd.$coursesearch.$licensesearch." order by u.firstname ASC";
		  }	
...
class iomad_completionstatus_filter_form extends moodleform {
...
if($param == 'organization'){
        ...
        $licenses =base::selectlicenses($departmentid);
		...
		$dateranges =array('datecomp'=>get_string('datecompleted', 'local_base'),'datereg'=>get_string('dateregistered', 'local_base'));
