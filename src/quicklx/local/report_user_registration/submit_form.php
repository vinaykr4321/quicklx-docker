		...
		if($format=='pdf'){
			...
			$tablepdf ='<table>
					<thead >
						...
						<th tyle="text-align:left;" >'.get_string('username', 'local_base').'</th>
		...
		if($format=='html'){
			...
			$filename = '';
                ...
                if($format=='pdf' ){
                        $fileLocation = $CFG->tempdir;
                        while (true) {
                                $filename = uniqid('userregistration', true) . '.pdf';
			...
			base::create_tempdf($reporthead,$html,$tablepdf,$filename);
                ...
                if($format =='csv' || $format =='xlsx'){
                        $fileLocation = $CFG->tempdir;
                        while (true) {
                                $filename = uniqid('userregistration', true) . '.csv';
                                if (!file_exists($fileLocation .'/'. $filename)) break;
                        }
                ...
                if($format =='xlsx')
                {
                    ...
                    $filename = "userregistration_". date('Y-m-d-H-i-s') . '.xlsx';
