...
iomad::require_capability('local/report_login_activity:view', $context);
...
// Url stuff.
$url = new moodle_url('/local/report_login_activity/index.php');
$dashboardurl = new moodle_url('/local/iomad_dashboard/index.php');

// Page stuff:.
$strcompletion = get_string('pluginname', 'local_report_login_activity');
$PAGE->set_url($url);
...
$url = new moodle_url('/local/report_login_activity/index.php', $params);
...
if ($exportuserdataobj) {
    ...
    $array_excel[] = array(get_string('username', 'local_base'),
    ...
    $tablepdf = '<table class="generaltable" id="ReportTable">
					...
					<th  class="header c0" style="text-align:center;" >' . get_string('username', 'local_base') . '</th>
					<th  class="header c1" style="text-align:center;" >' . get_string('firstname', 'local_base') . '</th>
					<th class="header c3" style="text-align:center;" >' . get_string('lastname', 'local_base') . '</th>
					<th class="header c4" style="text-align:center;" >' . get_string('sessionstart', 'local_base') . '</th>
					<th class="header c4" style="text-align:center;" >' . get_string('sessionend', 'local_base') . '</th>
					<th class="header c4" style="text-align:center;" >' . get_string('timeconnect', 'local_base') . '</th>
					</tr>
					</thead>
					<tbody> ';
    $i = 1;
        ...
        if($usersession->action == 'loggedin'){
            ...
            if($sessionend > 0){
                ...
                if($user){
                    ...
                    $array_excel[] = array($user->username,
                                                $user->firstname,
                                                $user->lastname,
                    ...
                    $tablepdf  .= '<tr style="'.$style.'">
                                    <td class="cell c0" style="text-align:left;">'.$user->username.' </td>
                                    <td  class="cell c1" style="text-align:center;">'.$user->firstname.'</td>
                                    <td class="cell c2" style="text-align:center;">'.$user->lastname.' </td>
                                    <td class="cell c4" style="text-align:left;">'.date('m/d/Y h:i:s A',base::usertimezonenextrun($sessionstart)).'</td>
                                    <td class="cell c4" style="text-align:left;">'.date('m/d/Y h:i:s A' ,base::usertimezonenextrun($sessionend)).'</td>
                                    <td class="cell c4" style="text-align:center;">'.$str_time.'</td>
                                    </tr>';
                    $i++;
                }   
            }
    ...
    //End custom changes March 25th, 2019
            $time = $sessionend - $sessionstart;
            $str_time = base::format_time($time);
	        ...
	        if($user){
                ...
                $tablepdf .= '<tr style="' . $style . '">
    								...
    								<td  class="cell c1" style="text-align:center;">' . $user->firstname . '</td>
    								<td class="cell c2" style="text-align:center;">' . $user->lastname . ' </td>
    								<td class="cell c4" style="text-align:left;">' . date('m/d/Y h:i:s A', $sessionstart) . '</td>
    								<td class="cell c4" style="text-align:left;">' . date('m/d/Y h:i:s A', $sessionend) . '</td>
    								<td class="cell c4" style="text-align:center;">' . $str_time . '</td>
    								</tr>';
                $i++;
            }
...
if ($userdataobj) {
    ...
    $coursecomptable->align = array('left', 'center', 'center', 'center', 'center', 'center', 'center', 'center');
    ...
    //Custom Code - Syllametrics - Updated (March 25, 2019)
    $hassessionend = true;
    ...
    foreach($userdataobj as $usersession){
        ...
        if($sessionend > 0){
            if ($row >= $rowstart && $printedrows < $rowend) {
                $userid=$usersession->userid;
                $user=$DB->get_record('user',array('id'=>$userid));
    //End custom changes on 25 th march,2019
                $isdata = 1;
                $time = $sessionend - $sessionstart;
                $str_time = base::format_time($time);
                if($user){
...
if ($isdata == 1) {
    ...
    $PAGE->set_url('/local/base/download_excel.php', array('name' => 'loginactivity'));
    ...
    $PAGE->set_url('/local/base/download_csv.php', array('name' => 'loginactivity'));
    ...
    $PAGE->set_url('/local/base/download_pdf.php', array('name' => 'loginactivity'));
    ...
    echo $output->paging_bar($totalcount, $page, $perpage, new moodle_url('/local/report_login_activity/index.php', $params));
    echo "<br />";
...
if ($exportuserdataobj) {
 ...
 <div class="modal fade" id="modalForm" role="dialog">
    ...
    echo ' </select>
                    ...
                    <div class="form-group">
                        ...
                        placeholder="Enter email subject" value="Login Activity report"/>
                    </div>
                    <div class="form-group">
 ...
 <div class="modal fade" id="schedulereportForm" role="dialog">
    ...
    echo' <input type="hidden" name="departmentid" value="' . $departmentid . '">
			<input type="hidden" name="reportname" value="login_activity">
			<input type="hidden" name="screen" value="1">
			  <div class="form-group">
                        <label for="description">Schedule Description:</label>
                        <input type="text" class="form-control" id="description" 
                        placeholder="Enter Description" value="Login Activity"/>
                    </div>
                      <div class="form-group">
                        <label >Report Format:</label><br>
    ...
    echo ' </select>
                     ...
                     <div class="form-group">
                        ...
                        placeholder="Enter email subject" value="Login Activity"/>
                    </div>
                    <div class="form-group">
