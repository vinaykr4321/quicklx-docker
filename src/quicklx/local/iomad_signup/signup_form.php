...
class iomad_signup_form extends moodleform {
    function definition() {
        ...
        if ($this->reg_key_status()) {
            $mform->addElement('text', 'regkey', get_string('regkey'), 'maxlength="100" size="25"');
        }
...
*/
    ...
    function validation($data, $files) {
        ...
        if (! validate_email($data['email'])) {
         ...
         if(!empty($data['regkey'])){
            
        $result = $this->validate_key($data['regkey'],$data['id']);
            if($result != 'true'){
                $errors['regkey'] =  $result;
            }
        }
    ...
    function reg_key_status()
    {
        global $DB;
        $id = required_param('id', PARAM_INT);
        $keyinfo = $DB->get_record_sql("SELECT enableregkey FROM {company} where id = $id");
        if ($keyinfo->enableregkey == '0') {
            return false;
        } else {
            return true;
        }
    ...
    function validate_key($keyvalue, $companyid)
    {
       ...
       // $keyvalue = urlencode($keyvalue);
        ...
        if ($keyexists) {
            ...
            if (isset($keyexists) && ($keyexists->expiry > time()) && ($keyexists->expiry != 'NULL')) {
                ...
                if ($keyexists->keytype == '1' && $keyexists->used_key == '1') {
                    return "1The key you entered could not be found. Please try again or contact support for help.";
                } elseif ($keyexists->keytype == '2' && $keyexists->noof_batch_uses_allowed <= $batchid->all_used) {
                    return "2The key you entered could not be found. Please try again or contact support for help.";
                }

                if ($valid != '1') {
                    return "3The key you entered could not be found. Please try again or contact support for help.";
                }
                return 'true';
            } else if(($keyexists->expiry < time()) && ($keyexists->expiry != 'NULL')){
                return "4The key you entered is expired. Please try a different key or contact support.";
            ...
            if (isset($keyexists) && ($keyexists->expiry == 'NULL')) {
                ...
                if ($keyexists->keytype == '1' && $keyexists->used_key == '1') {
                    return "5The key you entered could not be found. Please try again or contact support for help.";
                } elseif ($keyexists->keytype == '2' && $keyexists->noof_batch_uses_allowed <= $batchid->all_used) {
                    return "6The key you entered could not be found. Please try again or contact support for help.";
                }

                if ($valid != '1') {
                    return "7The key you entered could not be found. Please try again or contact support for help.";
                }
                return 'true';
            }
        }
        ...
        if (!$keyexists) {
            return "8The key you entered could not be found. Please try again or contact support for help.";
        }
    }
