...
if ($tagid) 
{
    $categoryid = getCategoryIdFromTagId($tagid);
}
...
function getCategoryIdFromTagId($tagid)
{
    global $DB;
    $catId = $DB->get_record_sql("SELECT c.category FROM {tag_instance} ti JOIN  {course} c ON c.id=ti.itemid WHERE ti.tagid=$tagid limit 1");
