...
$url = '';
if($companyId == 0){
    $customRedirect ="/login/index.php";
}else{
    $customRedirect ="/local/iomad_signup/login.php?id=$companyId&code=$code";
    $imagedata = $DB->get_record_sql("SELECT f.* FROM {files} f JOIN {company} c on c.splashimage = f.itemid WHERE c.id= $companyId AND mimetype !='' AND filearea = 'splashimage' ");
    ...
    if (!empty($imagedata)) 
    {
         ...
         $imagename = rawurlencode($imagedata->filename);
        $url = $CFG->wwwroot."/pluginfile.php/$imagedata->contextid/$imagedata->component/$imagedata->filearea/$imagedata->itemid/$imagedata->filepath/$imagename";
        echo '<input type="hidden" name="image" class="image" value="'.$url.'"';
    }
}
...
if (!empty($SESSION->loginerrormsg)) {
    ...
    redirect(new moodle_url($customRedirect));
