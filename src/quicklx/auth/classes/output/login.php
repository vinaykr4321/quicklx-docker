...
class login implements renderable, templatable {
    ...
    public function __construct(array $authsequence, $username = '') {
        ...
        if (isset($_GET['id'])) 
        {
            $id = $_GET['id'];
            $code = $_GET['code'];
            $this->forgotpasswordurl = new moodle_url("/login/forgot_password.php",array('id'=>$id,'code'=>$code));
        }
        else
        {
    ...
    public function export_for_template(renderer_base $output) {
        ...
        // Added by Syllametric Team -->
        $id = optional_param('id', 0, PARAM_INT);
        if($id != 0){
	    $customlogosignup = $DB->get_records_sql('select * from {files} WHERE itemid = ? AND filename != ? AND filearea = ?', array($id,'.','companylogo'));
            //$customlogosignup = $DB->get_record_sql('select * from {files} WHERE itemid = ? AND filename != ?', array($id,'.')); 
            if (!empty($customlogosignup)) 
            {
		// JD - Dec 16 2018 - mdl_files keeps old file records so have to pop latest item uploaded
		$customlogosignup = array_pop($customlogosignup);
                $customlogosignup = moodle_url::make_file_url("$CFG->wwwroot/pluginfile.php", "/$customlogosignup->contextid/$customlogosignup->component/$customlogosignup->filearea/$customlogosignup->itemid/".$customlogosignup->filename);
                $data->customlogosignup = $customlogosignup;
            }
	    $code = $_GET['code'];
            $data->loginurl = $CFG->wwwroot."/login/index.php?id=".$id."&code=".$code;

            $companycolours = $DB->get_record_sql('SELECT loginbackground,loginbutton FROM {company} WHERE id = ?', array($id));
            $data->loginbackground = $companycolours->loginbackground;
            $data->loginbutton = $companycolours->loginbutton;
        }else{
	    // JD - Dec 16 2018 - mdl_files keeps old file records so have to pop latest item uploaded
            $customlogosignup = $DB->get_records_sql("SELECT * FROM mdl_files WHERE contextid = ? AND component = ? AND filearea = ? AND itemid = ? AND filename != ? AND filesize != ? ",array('1','core_admin','logo','0','.','0'));
            if (!empty($customlogosignup))
            {
                $customlogosignup = array_pop($customlogosignup);
            	$customlogosignup = moodle_url::make_file_url("$CFG->wwwroot/pluginfile.php", "/$customlogosignup->contextid/$customlogosignup->component/$customlogosignup->filearea/$customlogosignup->itemid".'x200'."/$customlogosignup->timecreated/".$customlogosignup->filename);
            	$data->customlogosignup = $customlogosignup;
	    }
        }
        return $data;
