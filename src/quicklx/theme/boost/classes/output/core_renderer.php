...
class core_renderer extends \core_renderer {
    ...
    public function render_login(\core_auth\output\login $form) {
        ...
        */
        ...
        $companyshort = optional_param('code', 0, PARAM_CLEAN);
        $company = $DB->get_record('company', array('id'=> $companyid, 'shortname'=>$companyshort));

        if (($companyid != '0')) 
        {
            $logoData = $DB->get_record_sql("SELECT mimetype, contextid, component, filearea, itemid, filepath, filename FROM {files}  WHERE itemid = $companyid AND filesize != 0 AND mimetype LIKE '%image%' ");
            if (!empty($logoData)) 
            {
              if (substr($logoData->mimetype,0,5) == "image") 
              {
                $url = "$CFG->wwwroot/pluginfile.php/$logoData->contextid/$logoData->component/$logoData->filearea/$logoData->itemid/$logoData->filepath/$logoData->filename";
              }
