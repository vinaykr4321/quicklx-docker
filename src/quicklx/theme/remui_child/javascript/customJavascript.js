/*$.ajax({
    url: window.location.origin + "/logincheck.php",
    success: function (data) {
        var currUrl = window.location.href;
        //var urlParams = new URLSearchParams(window.location.search);
        //var code = urlParams.get('code');
        //var id = urlParams.get('id');
        if (data == 'false' && (currUrl.indexOf(window.location.origin + '/login/index.php') != 0 || currUrl.indexOf('forgot_password') != 0))
        {
...
require(['core/config'], function (mdlcfg) {
      ...
      /*  $.ajax({
            ...
            success: function (data) {
                var currUrl = window.location.href;
                if (data != 'true' && currUrl.indexOf(mdlcfg.wwwroot + '/login/index.php?') != 0 && currUrl.indexOf(mdlcfg.wwwroot + '/local/iomad_signup/login.php?') != 0)
                {
                    if (currUrl.indexOf(mdlcfg.wwwroot + '/login/forgot_password.php') != 0)
                    {
                        ...
                        if (pms[0] != '' && pms[0] != undefined && pms[1] != '' && pms[1] != undefined)
                        {
                            ...
                            if (currUrl != mdlcfg.wwwroot+'/' && company_id == undefined)
                            {
                                if(currUrl.split('.')[0].split('//')[1] != 'learning')
                                {
                                    window.location.href = mdlcfg.wwwroot;
                                }
                            }
                        }
                    }
...
//    }
    ...
    if ($('body').attr('id') == 'page-my-index')
    {
        function showcourses(maxcount) {
            ...
            $('.tab-pane').each(function () {
                ...
                {
                    $('#course-overview-section').siblings().find('.active').children().find('.hidden').each(function(){
                        if ($(this).hasClass('course-scroll') && count <= 16)
                        {
                            $(this).removeClass('hidden');
                            count = count + 1;
                        } else {
    ...
    var i=0;
    $('#page-blocks-iomad_company_admin-uploaduser .dep_tree').each(function(){
	    if(i==1){
		    $(this).prev().hide();
		    $(this).hide();
	    }
    ...
    /*code to get the selected userid from selection and set in the form variable*/
    $('#page-blocks-iomad_company_admin-company_license_users_form #remove').click(function () {
        var userids = [];
        $.each($('#currentlyenrolledusers option:selected'), function () {
            userids.push($(this).val());
        });

        var useridsJson = JSON.stringify(userids);
        $('#currentlyenrolledusers_ids').val(useridsJson);
    });
    ...
    $("#page-blocks-iomad_company_admin-company_edit_form #id_companyadvanced .fcontainer .form-group:nth-child(8) .col-md-9.checkbox").find('label').after("<span style='padding-left:20px' id=keygen />");
    require(['core/config'], function (mdlcfg) {
        var link = mdlcfg.wwwroot;
        $("#page-blocks-iomad_company_admin-company_edit_form #keygen").append('<span id = "showlink"><a href="' + link + '/local/report_createkeys/index.php">Generate New Key Batch</a><span/>');
        $("#page-blocks-iomad_company_admin-company_edit_form #keygen").append('<span id = "removelink">Generate New Key Batch</span>');
     ...
     $('input[type="checkbox"]').click(function(){
            if($(this).prop("checked") == true){
                $("#page-blocks-iomad_company_admin-company_edit_form #removelink").hide();
                $("#page-blocks-iomad_company_admin-company_edit_form #showlink").show();
            }
            else if($(this).prop("checked") == false){
               $("#page-blocks-iomad_company_admin-company_edit_form #showlink").hide();
               $("#page-blocks-iomad_company_admin-company_edit_form #removelink").show();
