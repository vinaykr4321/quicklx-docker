...
$templatecontext['leftcolumncolor'] = '';
global $DB;
$companyid = iomad::companyid();
if ($companyid !=0) 
{
    $companyid = iomad::get_my_companyid(context_system::instance());
    $colour = $DB->get_record_sql("SELECT leftcolumncolor FROM {company} WHERE id=$companyid");
    if (!empty($colour)) 
    {
        if ($colour->leftcolumncolor != null) 
        {
            $templatecontext['leftcolumncolor'] = $colour->leftcolumncolor;
        }
    }
}
...
$templatecontext['flatnavigation'] = child_flatnav_icon_support($PAGE->flatnav);
