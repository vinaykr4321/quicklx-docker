...
/*        if (!empty($category)) {
        ...
        if ($totalcount) {
            ...
            /*$tagId = $category;
            $courses = utility::getCoursesFromTagId($tagId);
*/
            $tagId = $category;
            ...
            {
                $courses = utility::getCoursesFromTagId($tagId,$search);
            }
    ...
    public static function get_course_category_selector($category = '', $categorysort = '', $search = '', $mycourses = '', $pageurl)
    {
        ...
        $licensed_desc = '';
        $licensed_asc = '';
        $licensed_high = ''; // high to low
        $licensed_low = ''; // low to high
        ...
        if ($categorysort == 'SORT_ASC') {
            ...
            $licensed_desc = 'selected';
        } elseif ($categorysort == 'LICENSED_ASC') {
            $licensed_asc = 'selected';
        } elseif ($categorysort == 'LICENSED_HIGH') {
            $licensed_high = 'selected';
        } elseif ($categorysort == 'LICENSED_LOW') {
            $licensed_low = 'selected';
        } else {
            $default = 'selected';
        }
        ...
        $tags = utility::getTagsAssignedToCourse($companyid);
        ...
        $categoryhtml .= "<label for='categoryselect' class='d-none'>".get_string('category', 'theme_remui')."</label> <select onchange='this.form.submit()' class='custom-select' id='categoryselect' name='categoryid' id='category'>
                <option value=''>".get_string('alltags', 'theme_remui')."</option>";
////////////////////////////////////////////////////////////////////////////////////////////////////////////////                
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        foreach ($tags as $key => $tag) {
            if ($category == $key) {
                $categoryhtml .= "<option selected value='{$key}'>{$tag}</option>";
            } else {
                $categoryhtml .= "<option value='{$key}'>{$tag}</option>";
            }
        }
        $categoryhtml .= "</select>";
        $categoryhtml .= "</div>";
        ...
        // Syllametrics - Jan 22 2019 - Can't assume licenses are specific to a user's company - needs to be off if only showing assigned courses	
	//$categoryhtml .= "<option ".$site_asc." value='SORT_ASC'>".get_string('sortascending', 'theme_remui')."</option>";
        ...
        // Show these options to manager and student
        if (!is_siteadmin()) 
        {
            $categoryhtml .= "<option ".$licensed_asc." value='LICENSED_ASC'>".get_string('licensed_asc', 'theme_remui_child')."</option>";
            $categoryhtml .= "<option ".$licensed_desc." value='LICENSED_DESC'>".get_string('licensed_desc', 'theme_remui_child')."</option>";
        }

        //show these options only to managers
        if (checkIsUserManager($USER->id)) 
        {
            $categoryhtml .= "<option ".$licensed_high." value='LICENSED_HIGH'>".get_string('licensed_high', 'theme_remui_child')."</option>";
            $categoryhtml .= "<option ".$licensed_low." value='LICENSED_LOW'>".get_string('licensed_low', 'theme_remui_child')."</option>";
        }
    ...
    public static function getTagsAssignedToCourse($companyid)
    {
        global $DB;
        $tags = array();
        $allTags = $DB->get_records_sql("SELECT ti.id,ti.tagid,t.rawname FROM {tag} AS t JOIN {tag_instance} AS ti ON t.id = ti.tagid JOIN {companylicense} cl JOIN {companylicense_courses} clc ON cl.id=clc.licenseid AND clc.courseid=ti.itemid  WHERE ti.itemtype='course' AND cl.companyid=$companyid");
        //$allTags = $DB->get_records_sql('SELECT ti.id,ti.tagid,t.rawname FROM {tag} AS t JOIN {tag_instance} AS ti ON t.id = ti.tagid WHERE ti.itemtype="course"');
        foreach ($allTags as $tag) 
        {
            $tags[$tag->tagid] = $tag->rawname;
        }
        return $tags;
    }

    public static function getCoursesFromTagId($tagId,$search)
    {
        ...
        if ($tagId == NULL) 
        {
            ...
            if (is_siteadmin()) 
            {
                if (!empty($search)) 
                {
                    ...
                    //$allCourses = $DB->get_records_sql("select * from {course} where id!=1 AND fullname like '%$search%'"); 
                
                    $allCourses = $DB->get_records_sql("SELECT c.* FROM {course} AS c JOIN {tag_instance} AS ti ON c.id=ti.itemid JOIN {tag} AS tag ON tag.id=ti.tagid WHERE ( replace(tag.name , ' ','')  like '%$search%' OR replace(c.fullname , ' ','') like '%$search%')"); 
                    $allCourses +=  $DB->get_records_sql("SELECT c.* FROM {course} AS c JOIN {tag_instance} AS ti ON c.id!=ti.itemid  WHERE replace(c.fullname , ' ','') like '%$search%'"); 
            ...
            else
            {
                ...
                if (!empty($search)) 
                {
                    if (checkIsUserManager($USER->id)) 
                    {
                        ...
                        //$allCourses = $DB->get_records_sql("SELECT * from {course} WHERE fullname like '%$search%'");  
                        //$DB->set_debug(true);
                        ...
                        //$allCourses = $DB->get_records_sql("SELECT c.* from {course} as c JOIN {company_course} as cc ON c.id = cc.courseid JOIN {company_users} as cu ON cu.companyid = cc.companyid where cc.companyid = $companyid and cu.userid = $USER->id AND c.fullname like '%$search%'");  
                    }
                    else
                    {
                        //For student
                        
                        //$allCourses = $DB->get_records_sql("SELECT c.* FROM {companylicense} cl JOIN {companylicense_courses} clc ON cl.id=clc.licenseid JOIN {companylicense_users} clu ON cl.id=clu.licenseid JOIN {course} c ON c.id=clc.courseid  WHERE cl.companyid = $companyid AND clu.userid = $USER->id AND c.fullname like '%$search%'");
//$DB->set_debug(true);
                       
                        $allCourses = $DB->get_records_sql("SELECT c.* FROM mdl_companylicense cl JOIN mdl_companylicense_courses clc ON cl.id=clc.licenseid JOIN mdl_course as c ON c.id=clc.courseid JOIN mdl_tag_instance AS ti ON c.id=ti.itemid JOIN mdl_tag AS tag ON tag.id=ti.tagid WHERE cl.companyid = $companyid AND (replace(tag.name , ' ','')  like '%$search%' OR replace(c.fullname , ' ','') like '%$search%')");

                        $allCourses += $DB->get_records_sql("SELECT c.* FROM mdl_companylicense cl JOIN mdl_companylicense_courses clc ON cl.id=clc.licenseid JOIN mdl_course as c ON c.id=clc.courseid JOIN mdl_tag_instance AS ti ON c.id!=ti.itemid JOIN mdl_tag AS tag ON tag.id=ti.tagid WHERE cl.companyid = $companyid AND replace(c.fullname , ' ','') like '%$search%' ");
                        //$allCourses = $DB->get_records_sql("SELECT c.* FROM {companylicense} cl JOIN {companylicense_courses} clc ON cl.id=clc.licenseid JOIN {companylicense_users} clu ON cl.id=clu.licenseid JOIN {course} c ON c.id=clc.courseid JOIN {tag_instance} AS ti ON c.id=ti.itemid JOIN {tag} AS tag ON tag.id=ti.tagid WHERE cl.companyid = $companyid AND clu.userid = $USER->id AND (tag.name  like '%$search%' OR replace(c.fullname , ' ','') like '%$search%')");

                        //$allCourses +=  $DB->get_records_sql("SELECT c.* FROM {companylicense} cl JOIN {companylicense_courses} clc ON cl.id=clc.licenseid JOIN {companylicense_users} clu ON cl.id=clu.licenseid JOIN {course} c ON c.id=clc.courseid JOIN {tag_instance} AS ti ON c.id!=ti.itemid  WHERE cl.companyid = $companyid AND clu.userid = $USER->id AND replace(c.fullname , ' ','') like '%$search%'");
                ...
                {
                    $allCourses = $DB->get_records_sql("SELECT * from {course} as c JOIN {company_course} as cc ON c.id = cc.courseid JOIN {company_users} as cu ON cu.companyid = cc.companyid where cc.companyid = $companyid and cu.userid = $USER->id");  
                }
            }
        ...
        {
            /*$allCourses = $DB->get_records_sql("SELECT c.* FROM {course} AS c JOIN {tag_instance} AS ti ON c.id = ti.itemid WHERE ti.tagid=$tagId AND c.fullname like '%$search%'");*/

            $allCourses = $DB->get_records_sql("SELECT c.* FROM {course} AS c JOIN {tag_instance} AS ti ON c.id=ti.itemid JOIN {tag} AS tag ON tag.id=ti.tagid WHERE ti.tagid=$tagId AND ( replace(tag.name , ' ','') like '%$search%' OR replace(c.fullname , ' ','') like '%$search%')");
        }        
        return $allCourses;
    }
    ...
    {
        global $DB;
        $data = $DB->get_records_sql("SELECT r.id from mdl_role_assignments ra JOIN mdl_role r on ra.roleid = r.id where ra.userid=$userid");
