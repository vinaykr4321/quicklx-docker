...
class core_renderer extends remui_renderer {
    ...
    public function should_display_logo() {
        ...
        $context = array('islogo' => false, 'issitename' => false, 'isiconsitename' => false, 'isexpandlogo' => false);
         ...
         * Last Update : 8th Nov, 2018
         * Change logo as per selected company
        */
        
            if (\iomad::companyid() != 0 || isset($_GET['id'])) 
            {
                if (\iomad::companyid() == 0) 
                {
                    $companyid = $_GET['id'];
                }
                else
                ...
                /* Added Dec 13th for ThinkTeach release - can encapsulate later */
                $currenturl = 'http://' . $_SERVER['SERVER_NAME'] . $_SERVER['REQUEST_URI'];
                /*if ($companyid && ($companyid == 27 || (stripos($currenturl,'thinkteach') !== false))) {
                        $context['isexpandlogo'] = true;
		}*/

                $logoData = $DB->get_record_sql("SELECT f.mimetype,f.contextid,f.component,f.filearea,f.itemid,f.filepath,f.filename,c.headingcolor,c.maincolor,c.linkcolor from {files} as f JOIN {company} as c ON f.itemid = c.id where f.itemid = '$companyid' AND f.filesize != 0 AND f.mimetype like '%image%' AND f.filearea like '%companylogo%' ");
                if (!empty($logoData)) 
                {
                  if (substr($logoData->mimetype,0,5) == "image") 
                  {
                    $context['newlogo'] = "$CFG->wwwroot/pluginfile.php/$logoData->contextid/$logoData->component/$logoData->filearea/$logoData->itemid/$logoData->filepath/$logoData->filename";
                  }
                  $context['headingcolor'] = $logoData->headingcolor;
                  $context['maincolor'] = $logoData->maincolor;
                  $context['linkcolor'] = $logoData->linkcolor;
                  $param = '';
                  if (isset($_GET['id'])) 
                  {
                    $id = $_GET['id'];
                  ...
                  $context['customurl'] = $CFG->wwwroot."$param";
                }
                else
                {
                  $color = $DB->get_record_sql("SELECT headingcolor,maincolor,linkcolor from {company} where id = '$companyid' ");
                  $context['headingcolor'] = $color->headingcolor;
                  $context['maincolor'] = $color->maincolor;
                  $context['linkcolor'] = $color->linkcolor;
                }
            }
        /* Custom code*/
    ...
    /*public function get_iomad_logo_url($maxwidth = 100, $maxheight = 100) {
      global $CFG;
