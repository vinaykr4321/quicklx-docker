...
class grade_report_overview extends grade_report {
    ...
    public function fill_table($activitylink = false, $studentcoursesonly = false) {
        ...
        if ($this->courses) {
            ...
            foreach ($coursesdata as $coursedata) {
                ...
                // Handle expired courses
                $companyid = iomad::get_my_companyid(context_system::instance()); 
                $licenseexpire = self::getCompanyLicenseExpiryId($companyid, $course->id);
    ...
    // Syllametrics (support@syllametrics.com)
    // Last update: May 2019
	public function getCompanyLicenseExpiryId($courseid) {
	    global $DB,$USER;

        $expicourseId = $DB->get_record_sql("SELECT e.courseid,ue.* FROM {user_enrolments} ue join {enrol} e ON ue.enrolid = e.id where ue.userid = $USER->id AND e.courseid = $courseid AND  ue.timeend != 0");
