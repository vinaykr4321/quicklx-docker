        ...
        if ($userids = $DB->get_records_sql("SELECT ue.id, ue.userid, ue.enrolid, e.courseid
            ...
            foreach ($userids as $user) {
                ...
                if ($gradeitems = $DB->get_records_sql("SELECT gg.id, gg.finalgrade, gg.feedback, gi.itemtype
                    ...
                    // Removed the delete query after changes requested in Issue #397 Item H
                    //mtrace("removing grade items from course $user->courseid");
                    mtrace("**skipped** removing grade items from course $user->courseid");
                    ...
                    mtrace("Keeping course completion for user $user->userid on $user->courseid");
                    ...
                    mtrace("Keeping enrolment for user ".$user->userid." from ".$user->courseid);
